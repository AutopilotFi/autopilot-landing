...
export default function AppPreview() {
  ...
  const stats = [
    {
      label: "Total Balance",
      value: currentData.autopilotBalance.toLocaleString('en-US', { maximumFractionDigits: 0 }),
      unit: currentData.asset
    },
    {
      label: "Total Earnings",
      value: currentData.currentEarnings.toLocaleString('en-US', { maximumFractionDigits: 0 }),
      unit: currentData.asset
    },
    {
      label: "Monthly Forecast",
      value: currentData.monthlyForecast.toLocaleString('en-US', { maximumFractionDigits: 0 }),
      unit: currentData.asset
    },
    {
      label: "Next Update",
  ...
  const chartData = [
    { x: 10, y: 100 }, { x: 75, y: 85 }, { x: 140, y: 70 }, { x: 205, y: 60 },
    { x: 270, y: 45 }, { x: 335, y: 30 }, { x: 390, y: 15 }
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 lg:mb-16">
          ...
          <p className="max-w-2xl mx-auto text-lg lg:text-xl text-gray-600">
            Your single entry point to Morpho&apos;s best-performing, curated vaults. Monitor performance and earnings in real time.
          </p>
        </div>
            ...
            <div className="bg-gray-100 px-4 py-3 flex items-center justify-between border-b border-gray-200">
              <div className="flex items-center space-x-3">
                ...
                <div className="flex space-x-2">
                  ...
                  <div className="w-3 h-3 bg-yellow-500 rounded-full hover:bg-yellow-600 transition-colors cursor-pointer animate-pulse" style={{animationDelay: '0.2s'}}></div>
                  <div className="w-3 h-3 bg-[#9159FF] rounded-full hover:bg-[#7c3aed] transition-colors cursor-pointer animate-pulse" style={{animationDelay: '0.4s'}}></div>
                </div>
                ...
                <div className="flex-1 max-w-lg mx-4 hidden sm:block">
                  <div className="bg-white rounded-md px-4 py-2 text-sm text-gray-700 border border-gray-300 shadow-inner">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-mono text-xs lg:text-sm">https://app.autopilot.finance</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white min-h-[500px] lg:min-h-[600px]">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3 lg:space-x-4">
                      ...
                      <div className="flex items-center space-x-2 lg:space-x-3">
                        <Image src={currentData.assetIcon} width={28} height={28} alt={currentData.asset} className="w-6 h-6 lg:w-8 lg:h-8" />
                        <div>
                          <h1 className="text-sm lg:text-lg font-semibold text-gray-900">{currentData.asset} Autopilot</h1>
                          <div className="flex items-center space-x-1 lg:space-x-2">
                            <Image src={currentData.icon} width={14} height={14} alt={currentData.name} className="w-3 h-3 lg:w-4 lg:h-4" />
                            <span className="text-xs lg:text-sm text-gray-500">{currentData.name}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2 lg:space-x-4">
                      <div className="flex items-center space-x-1 lg:space-x-2 bg-purple-50 border border-purple-200 px-2 lg:px-3 py-1 lg:py-1.5 rounded-md">
                        ...
                        <span className="text-xs lg:text-sm font-medium text-purple-700">Live</span>
                      </div>
              ...
              <div className="bg-white border-b border-gray-100 px-4 lg:px-6 overflow-x-auto">
                <div className="flex space-x-4 lg:space-x-8 min-w-max">
                  {['Overview', 'Deposit', 'Earnings', 'Benchmark', 'Details', 'History'].map((tab, index) => (
                    <button
                      key={tab}
              ...
              <main className="p-4 lg:p-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6 mb-6 lg:mb-8">
                  {stats.map((stat, index) => (
                    <div key={index} className="bg-white rounded-lg border border-gray-100 p-3 lg:p-6 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between mb-2 lg:mb-3">
                        <p className="text-xs lg:text-sm font-medium text-gray-600">{stat.label}</p>
                        {stat.unit !== 'min' && (
                          <Image src={currentData.assetIcon} height={14} width={14} alt={stat.unit} className="w-3 h-3 lg:w-4 lg:h-4" />
                        )}
                      </div>
                      <div className="flex items-baseline space-x-1 lg:space-x-2">
                        <span className="text-lg lg:text-2xl font-bold text-gray-900">{stat.value}</span>
                        <span className="text-xs lg:text-sm text-gray-500">{stat.unit}</span>
                      </div>
                    </div>
                  ))}
                </div>
                        ...
                        {['7d', '1m', 'all'].map((period, index) => (
                          <button
                            ...
                            className={`px-2 lg:px-3 py-1.5 text-xs lg:text-sm font-medium rounded transition-colors ${
                              index === 2 ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                            }`}
                          >
                            {period}
                          </button>
                        ...
                        <svg className="w-full h-full" viewBox="0 0 400 120" preserveAspectRatio="none">
                          <defs>
                            <linearGradient id="earningsGradient" x1="0%" y1="0%" x2="0%" y2="100%">
                              <stop offset="0%" stopColor="#9159FF" stopOpacity="0.2"/>
                              <stop offset="100%" stopColor="#9159FF" stopOpacity="0"/>
                            </linearGradient>
                          </defs>
                  ...
                  <div className="bg-white rounded-lg border border-gray-100 p-4 lg:p-6 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="space-y-3 lg:space-y-4">
                      {currentData.recentEarnings.map((earning, index) => (
                        <div key={index} className="flex items-center justify-between py-2 lg:py-3 px-2 lg:px-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
                          <div className="flex items-center space-x-2 lg:space-x-3">
                            <Image src={currentData.assetIcon} width={17.5} height={17.5} alt={currentData.asset} className="w-4 h-4 lg:w-5 lg:h-5" />
                            <div>
        ...
        <div className="text-center mt-12 lg:mt-16">
          ...
          >
            ...
            <Image src="/launchIcon.png" alt="Launch Icon" width={17.5} height={17.5} className="w-5 h-5" />
          </button>
        </div>
      </div>
    </section>
